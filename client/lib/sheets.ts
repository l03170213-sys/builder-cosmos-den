...
export async function fetchRespondentOverallFromMatrice(sheetId: string, gidMatrice?: string, params?: { email?: string; name?: string; date?: string }) {
  const res = await fetchRespondentDetailsFromSheet(sheetId, gidMatrice, params);
  return res ? (res.overall ?? res.overallAverage ?? null) : null;
}

export async function fetchRespondentDetailsFromSheet(sheetId: string, gidMatrice?: string, params?: { email?: string; name?: string; date?: string }) {
  ...
  try {
    ...
    const normalizeDiacritics = (s:string) => {
      try { return s.normalize('NFD').replace(/\p{Diacritic}/gu,'').replace(/\s+/g,' ').trim().toLowerCase(); } catch(e) { return s.toString().trim().toLowerCase(); }
    };

    // Try columns-as-respondents first
    const headerIndexMap: Record<string, number> = {};
    for (let ci=0; ci<mcols.length; ci++) {
      const key = normalizeDiacritics((mcols[ci]||'').toString()).replace(/[^a-z0-9@]+/g,'');
    ...
    for (const lblKey of Object.keys(headerIndexMap)) {
      if (targetEmail && lblKey.includes(targetEmail)) { respColIndex = headerIndexMap[lblKey]; break; }
    ...
    if (respColIndex === -1 && mcols && mcols.length) {
      const nameTokens = (nameKey||'').split(/\s+/).map(t=>normalizeDiacritics(t).replace(/[^a-z0-9]+/g,'')).filter(Boolean);
      for (let ci=0; ci<mcols.length; ci++) {
        const lbl = normalizeDiacritics(mcols[ci]||'').replace(/[^a-z0-9]+/g,'');
        if (!lbl) continue;
        if (targetEmail && lbl.includes(targetEmail)) { respColIndex=ci; break; }
        let matchCount=0; for (const tk of nameTokens) if (lbl.includes(tk)) matchCount++;
    ...
    if (respColIndex !== -1) {
      ...
      for (let i=1;i<=10;i++) {
        ...
        const catNameFromRow = foundRow && foundRow.c && foundRow.c[0] ? cellToStringLocal(foundRow.c[0]) : '';
      ...
      for (let ri=0; ri<mrows.length; ri++) {
        const first = mrows[ri] && mrows[ri].c && mrows[ri].c[0] ? cellToStringLocal(mrows[ri].c[0]) : '';
      ...
      if (!overallVal) {
        for (let ri=mrows.length-1; ri>=0; ri--) {
          ...
          if (c && c.v != null) { overallVal = parseRatingCell(c); break; }
        }
      }
      return { categories: [{ name: 'Nom', value: '' }, ...newCats], overall: overallVal, column: String(respColIndex) };
    }
    ...
    for (let ri=0; ri<mrows.length; ri++) {
      ...
      const norm = cells.map((c:any)=>normalizeDiacritics(cellToStringLocal(c)||'').replace(/\s+/g,''));
      if (targetEmail && norm.some((v:any)=>v && v.includes(targetEmail))) { matchedRowIdx = ri; break; }
      const targetNameD = normalizeDiacritics(targetName).replace(/\s+/g,'');
      if (targetName && norm.some((v:any)=>v && (v.includes(targetNameD) || targetNameD.includes(v)))) { matchedRowIdx = ri; break; }
    }
    if (matchedRowIdx !== -1) {
      ...
      for (let ci=1; ci<=Math.min(maxCols,20); ci++) {
        ...
        let val = '';
        if (mrow && mrow.c && mrow.c[ci] && mrow.c[ci].v != null) val = parseRatingCell(mrow.c[ci]);
        newCats.push({ name: headerName, value: val });
      }
      const overallCell = mrow && mrow.c && (mrow.c[11]) ? mrow.c[11] : null;
      const overall = overallCell && overallCell.v != null ? parseRatingCell(overallCell) : null;
      return { categories: [{ name: 'Nom', value: '' }, ...newCats], overall, column: null };
    }
    ...
    try {
      ...
      for (let ri=0; ri<srows.length; ri++) {
        ...
        const normCells = cells.map((c:any)=>(cellToStringLocal(c)||'').toString().trim().toLowerCase());
        if (targetEmail && normCells.some((v:any)=>v && v.includes(targetEmail))) { matchedRowIdx = ri; break; }
        if (targetName && normCells.some((v:any)=>v && v.includes(normalizeDiacritics(targetName)))) { matchedRowIdx = ri; break; }
      }
      if (matchedRowIdx !== -1) {
        ...
        for (const [catName, titles] of Object.entries(categoriesDef)) {
          ...
          for (let hi=0; hi<scols.length; hi++) {
            const h = scols[hi] || '';
            for (const t of titles) {
              if (h && typeof h==='string' && h.toString().trim().toLowerCase().includes(t.toLowerCase())) { colIndexes.push(hi); break; }
            }
          }
          if (colIndexes.length===0) { newCats.push({ name: catName, value: '' }); continue; }
          ...
          if (count>0) { const avg=sum/count; const norm=normalizeAverage(avg); newCats.push({ name: catName, value: norm!=null?String(Number(norm.toFixed(2))):'' }); } else { newCats.push({ name: catName, value: '' }); }
        }
        // overall from sheet1 if present
        const overall = (scells[11] && scells[11].v!=null) ? String(scells[11].v) : null;
        return { categories: [{ name: 'Nom', value: '' }, ...newCats], overall, column: null };
      }
    } catch (e) {
      // ignore
    ...
    try {
      const sres = await fetch(`https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq`);
      if (!sres.ok) return null;
      const st = await sres.text();
      const sj = parseGviz(st);
      const headers = (sj.table.cols||[]).map((c:any)=>(c.label||'').toString());
      ...
      for (const [catName,titles] of Object.entries(categoriesDef)) {
        ...
        for (let ri=0;ri<rows.length;ri++){
          ...
          for (const ci of colIndexes){ const raw=cells[ci] && cells[ci].v!=null?cells[ci].v:null; const n=toNumber(raw); if(n!=null){sum+=n;count++}}}
        if(count>0){ const avg=sum/count; const norm=normalizeAverage(avg); if(norm!=null) cats.push({ name:catName, average:norm}); }
      }
      const overallAverage = cats.length ? (cats.reduce((s:any,c:any)=>s+c.average,0)/cats.length) : 0;
